...
IoT_Error_t iot_tls_connect(Network *pNetwork, TLSConnectParams *params) {
   ...
   /*  Load root CA...
    ...
    uint32_t base_timeout = pNetwork->tlsConnectParams.timeout_ms ? pNetwork->tlsConnectParams.timeout_ms : IOT_SSL_READ_TIMEOUT;
    mbedtls_ssl_conf_read_timeout(&(tlsDataParams->conf), base_timeout);
    tlsDataParams->read_timeout_ms = base_timeout;
...
IoT_Error_t iot_tls_read(Network *pNetwork, unsigned char *pMsg, size_t len, Timer *timer, size_t *read_len) {
    ...
    size_t rxLen = 0;
    int ret;
    uint32_t base_timeout = tlsDataParams->read_timeout_ms ? tlsDataParams->read_timeout_ms : IOT_SSL_READ_TIMEOUT;

    while (len > 0) {
        uint32_t effective_timeout = MAX(1U, MIN(base_timeout, left_ms(timer)));
        mbedtls_ssl_conf_read_timeout(&(tlsDataParams->conf), effective_timeout);
