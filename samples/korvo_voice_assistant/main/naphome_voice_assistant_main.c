...
// WakeNet local control callback (parallel to OpenAI Realtime)
// This runs when WakeNet9l detects "hi esp" for local control
// OpenAI Realtime continues streaming in parallel
static void wakenet_local_control_cb(const char *wake_word, int word_index, void *ctx)
{
    ...
    // This runs in parallel with OpenAI Realtime streaming
    if (!s_muted && pipeline) {
        ...
        // capture audio and send to OpenAI, conflicting with continuous streaming.
...
void app_main(void)
{
    ...
    voice_pipeline_config_t pipeline_cfg = {
        ...
        .use_realtime_streaming = false,  // Gemini uses batch STT, not realtime streaming
    ...
    // Set WakeNet local control callback (parallel to OpenAI Realtime)
    ...
    if (pipeline_cfg.use_realtime_streaming) {
        if (pipeline_cfg.enable_wakenet_local) {
            ESP_LOGI(TAG, "Assistant ready: OpenAI Realtime streaming + WakeNet9l local control");
            ESP_LOGI(TAG, "  Pipeline: I2S -> AEC -> BSS/NS -> VAD -> OpenAI (continuous)");
            ESP_LOGI(TAG, "  Pipeline: I2S -> AEC -> BSS/NS -> WakeNet -> Local Control (triggered)");
            ESP_LOGI(TAG, "  Say '%s' for local control, or speak for OpenAI transcription", pipeline_cfg.wakenet_model);
        } else {
            ESP_LOGI(TAG, "Assistant ready with OpenAI Realtime streaming (AFE-enabled, no wake word)");
        }
    } else {
