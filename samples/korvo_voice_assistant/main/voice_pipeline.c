...
struct voice_pipeline {
    ...
    openai_realtime_handle_t realtime_handle;
...
static void voice_pipeline_process_interaction(voice_pipeline_handle_t handle)
{
    ...
    char transcript_text[OPENAI_MAX_TRANSCRIPT_CHARS] = {0};
    ...
    char filtered_text[OPENAI_MAX_TRANSCRIPT_CHARS] = {0};
    ...
    esp_err_t tts_err = openai_tts_generate(response_text,
...
static void voice_pipeline_process_button(voice_pipeline_handle_t handle, int button_id)
{
    ...
    char llm_text[OPENAI_MAX_TRANSCRIPT_CHARS] = {0};
    esp_err_t llm_err = openai_generate_text_response(prompt, llm_text, sizeof(llm_text));
    if (llm_err != ESP_OK || llm_text[0] == '\0') {
        snprintf(llm_text, sizeof(llm_text), "Button %d pressed.", button_id);
    }
    set_led_state(handle, LED_CONTROLLER_STATE_SPEAKING);
    size_t wav_bytes = 0;
    esp_err_t tts_err = openai_tts_generate(llm_text,
                                            handle->cfg.tts_voice,
                                            handle->tts_buffer,
                                            handle->tts_buffer_bytes,
                                            &wav_bytes);
...
static void gpt_tts_response_task(void *arg)
{
    ...
    char llm_response[OPENAI_MAX_TRANSCRIPT_CHARS] = {0};
...
static void realtime_transcript_cb(const char *text, bool is_final, void *ctx)
{
    ...
    if (is_final) {
        ...
        ESP_LOGI(TAG, "========================================");
        ESP_LOGI(TAG, "OpenAI STT (FINAL): \"%s\"", text);
        ESP_LOGI(TAG, "========================================");
    } else {
        // Partial transcription - incremental updates
        ESP_LOGI(TAG, "OpenAI STT (partial): \"%s\"", text);
    }
    
    if (is_final && strlen(text) > 0) {
        // Send STT transcription to GPT chat and TTS the response
        // Do this in a separate task to avoid blocking WebSocket callback
        ...
        if (!gpt_tts_task_data.active && strlen(text) < sizeof(gpt_tts_task_data.text)) {
            ...
            // Spawn task to handle GPT chat + TTS
            xTaskCreate(gpt_tts_response_task, "gpt_tts_task", 8192, &gpt_tts_task_data, 5, NULL);
        } else {
            ESP_LOGW(TAG, "GPT+TTS task already active, skipping");
        }
...
static void voice_pipeline_realtime_stream_task(void *arg)
{
    ...
    ESP_LOGI(TAG, "Starting continuous audio streaming with Gemini STT-LLM-TTS");
    ESP_LOGI(TAG, "Pipeline: I2S -> AEC -> BSS/NS -> VAD -> Gemini Batch STT -> Gemini LLM -> Gemini TTS");
    ESP_LOGI(TAG, "Gemini: Using batch STT (accumulates audio during speech, processes when speech ends)");
    // For Gemini, we use batch STT when VAD detects speech
    // Don't start realtime API (Gemini doesn't support it)
    handle->realtime_handle = NULL;
    
    int16_t *frame_buffer = malloc(512 * sizeof(int16_t));
    if (!frame_buffer) {
        ESP_LOGE(TAG, "Failed to allocate frame buffer");
        openai_realtime_stop(handle->realtime_handle);
        vTaskDelete(NULL);
        return;
    }
...
#if CONFIG_KORVO_FARFIELD_WAKE_WORD_ENABLE
        // Process through AFE pipeline: I2S -> AEC -> BSS/NS -> VAD -> OpenAI
            ...
            while (afe_accumulator_count >= samples_per_feed && samples_per_feed > 0) {
                ...
                if (feed_result >= 0) {
                    ...
                    if (fetch_result && fetch_result->ret_value == ESP_OK) {
                        ...
                        // Check WakeNet wake word detection (parallel to OpenAI)
                        // Dual pipeline:
                        //   Path 1: I2S -> AEC -> BSS/NS -> VAD -> OpenAI Realtime (continuous transcription)
                        ...
                        // Check VAD state - only send to OpenAI when speech is detected
                        ...
                        // Gemini: Accumulate audio during speech, batch STT when speech ends
                            if (vad_detected) {
                                ...
                                if (!handle->vad_was_active) {
                                    ESP_LOGI(TAG, "Gemini: Speech started, accumulating audio");
                                }
                                ...
                                ESP_LOGI(TAG, "Gemini: Speech ended (%zu samples), processing batch STT", handle->gemini_audio_samples);
                            ...
                            // OpenAI: Send to Realtime API when VAD detects speech
                            if (vad_detected && handle->realtime_handle) {
                                // Send AFE-processed audio (after AEC -> BSS/NS -> VAD) to OpenAI
                                ...
                                // Send processed audio to OpenAI Realtime
                                openai_realtime_send_audio(handle->realtime_handle, audio_to_send, samples_to_send);
                                
                                static int vad_log_count = 0;
                                if (++vad_log_count % 10 == 0) {  // Very frequent logging for testing
                                    ESP_LOGI(TAG, "✅ VAD ACTIVE: Pipeline I2S->AEC->BSS/NS->VAD->OpenAI (energy=%.1f, threshold=%.1f)", 
                                ...
                                if (++vad_silence_count % 20 == 0) {  // More frequent for testing
                                    ESP_LOGI(TAG, "⚠️ VAD INACTIVE: skipping OpenAI (energy=%.1f < threshold=%.1f)", 
                                            energy, vad_energy_threshold);
                                }
                            }
                        }
...
#endif
        {
            // No AFE - send raw audio directly to OpenAI
            // Resample to 24kHz if needed (simplified: just send as-is for now)
            if (handle->realtime_handle) {
                openai_realtime_send_audio(handle->realtime_handle, frame_buffer, samples_read);
            }
        }
...
voice_pipeline_handle_t voice_pipeline_create(const voice_pipeline_config_t *cfg)
{
    ...
    // Pipeline: I2S -> AEC -> BSS/NS -> VAD -> OpenAI Realtime
    if (cfg->use_realtime_streaming) {
        ESP_LOGI(TAG, "Initializing AFE pipeline: I2S -> AEC -> BSS/NS -> VAD -> OpenAI Realtime");
        ...
        if (models && models->num > 0) {
            ...
            if (model_name) {
                ...
                if (afe_config) {
                    ...
                    // 4. VAD: Voice Activity Detection (after NS, gates OpenAI)
                    //    Only send audio to OpenAI when speech is detected
                    ...
                    // OpenAI Realtime handles transcription, WakeNet handles local commands
                    if (cfg->enable_wakenet_local && cfg->wakenet_model) {
                        ...
                        ESP_LOGI(TAG, "WakeNet disabled - using VAD to gate OpenAI only");
                    }
