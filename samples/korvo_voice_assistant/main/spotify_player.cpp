...
class SpotifyPlayerTask : public bell::Task {
  public:
    explicit SpotifyPlayerTask(const spotify_player_config_t &cfg)
        : bell::Task("cspot", 32 * 1024, 0, 1), cfg_(cfg)
    {
...
extern "C" esp_err_t spotify_player_start(const spotify_player_config_t *config)
{
    ...
    try {
        s_task = new SpotifyPlayerTask(*config);
        ESP_LOGI(TAG, "SpotifyPlayerTask created successfully");
        return ESP_OK;
    } catch (const std::exception& e) {
        ESP_LOGE(TAG, "Failed to create SpotifyPlayerTask: %s", e.what());
        return ESP_FAIL;
    } catch (...) {
        ESP_LOGE(TAG, "Failed to create SpotifyPlayerTask: unknown exception");
        return ESP_FAIL;
    }
#else
    (void)config;
