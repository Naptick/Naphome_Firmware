...
{
    size_t len = strlen("X-Goog-Api-Key: ") + strlen(GEMINI_API_KEY_STRING) + 1;
    char *auth = malloc(len);
    if (!auth) {
        return NULL;
    }
    snprintf(auth, len, "X-Goog-Api-Key: %s", GEMINI_API_KEY_STRING);
    return auth;
}
...
esp_err_t gemini_transcribe_wav(const int16_t *pcm_samples, size_t sample_count, int sample_rate_hz, gemini_transcription_t *result)
{
    ...
    ESP_LOGI(TAG, "🔊 [Gemini STT] Sending HTTP POST request...");
